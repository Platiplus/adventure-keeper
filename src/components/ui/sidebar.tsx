    ...
    <div className="relative min-h-screen pb-16">
      <div className="fixed bottom-4 right-4 z-10">
        <Button
          ...
          variant="outline"
          size="icon"
          className={cn('h-7 w-7 bg-primary-foreground border-4 border-primary py-6 px-6 rounded-full', className)}
          onClick={(event) => {
            onClick?.(event)
            toggleSidebar()
          }}
...
>(({ asChild = false, isActive = false, variant = 'default', size = 'default', tooltip, className, ...props }, ref) => {
  ...
  const button = (
    <Comp ref={ref as any} data-sidebar="menu-button" data-size={size} data-active={isActive} className={cn(sidebarMenuButtonVariants({ variant, size }), className)} {...props} />
  )
