...
export const TagsApi = async () => {
  ...
  const getById = async (id: string): Promise<Tag | null> => {
    const tag = await client.from('tags').select('*').eq('id', id).single()

    return tag.data ?? null
  }

  const getBySlug = async (slug: string): Promise<Tag | null> => {
    const tag = await client.from('tags').select('*').eq('slug', slug).single()

    return tag.data ?? null
  }

  const listAll = async (): Promise<Tag[]> => {
    const tags = await client.from('tags').select('*')

    return tags.data ?? []
  }

  const listBySlug = async (slugs: string[]): Promise<Tag[]> => {
    const tags = await client.from('tags').select('*').in('slug', slugs)

    return tags.data ?? []
  }

  const listAllWithFilters = async (filters: Partial<Tag>): Promise<Tag[]> => {
    const tags = await client.from('tags').select('*').eq('filters', filters)
